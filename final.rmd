...
Meters data
```{r}
meter_trans = read.csv("/Users/inordia/Desktop/UPenn搞起来/592/pro-forma/data/meter_trans_2021-09.csv")%>%mutate(
...
```{r}
parking.time.panel = read.csv("/Users/inordia/Desktop/UPenn搞起来/592/pro-forma/data/parking_spot_panel.csv")
...
parking.time.panel.sum %>%
  ...
  group_by(interval60, street_seg_ctrln_id, time_of_day) %>%
  tally()%>%
  group_by(street_seg_ctrln_id, time_of_day)%>%
  summarize(mean_time = mean(parking_time_in_60m))%>%
  ggplot()+
  geom_histogram(aes(mean_time), binwidth = 50)+
...
reg1 <- lm(formula1, data=ride.Train)
reg2 <- lm(formula2, data=ride.Train)
reg3 <- lm(formula3, data=ride.Train)
reg4 <- lm(formula4,  data=ride.Train)
