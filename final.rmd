...
parking.time.panel.sum<-
  parking.time.panel.sum%>%
  mutate(status = ifelse(week<= 38, "train", "test"))
...
formula1 = parking_time_in_60m ~ PRKG_SPLY + Temperature+Percipitation+Wind_Speed

formula2 = parking_time_in_60m ~ PRKG_SPLY + lagHour + lag2Hours+lag3Hours+lag4Hours+lag1day+weekday+is.weekend+hour+Temperature+Percipitation+Wind_Speed

formula3 = parking_time_in_60m ~  PRKG_SPLY + Temperature+Percipitation+Wind_Speed +transit.nn +park_public.nn+park_public.nn+cultural_district.nn+restaurant.nn+cafe.nn+theatre.nn+clothes.nn+Temperature+Percipitation+Wind_Speed

formula4 = parking_time_in_60m ~  PRKG_SPLY + lagHour + lag2Hours+lag3Hours+lag4Hours+lag12Hours+lag1day+weekday+is.weekend+hour+transit.nn +park_public.nn+park_public.nn+cultural_district.nn+restaurant.nn+cafe.nn+theatre.nn+clothes.nn+Temperature+Percipitation+Wind_Speed
...
week_predictions <- 
  park.Test.weekNest %>% 
    mutate(Weather_model = map(.x = data, fit = reg1, .f = model_pred),
           Time_Lag_model = map(.x = data, fit = reg2, .f = model_pred),
           Space_Lag_model = map(.x = data, fit = reg3, .f = model_pred),
           Time_Space_Lag_model = map(.x = data, fit = reg4, .f = model_pred))
...
```{r}
# cross-validation
fitControl <- caret::trainControl(method = "cv", number = 30)
set.seed(1)
