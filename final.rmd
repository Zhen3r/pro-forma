...
parking_seg_census%>%
  ...
  ggplot()+
    ...
    scale_fill_manual(values=inferno(18)[7:13])+
...
mapdata = parking_seg_census%>%
  ...
  mutate_at(ends_with(".nn")%>%vars()%>%all_of(),log) %>% 
  pivot_longer(ends_with(".nn")) %>% 
  filter(name=="transit.nn")%>%
  mutate(name=as.factor(1))%>%
  st_sf()
...
```{r}
# cross-validation
fitControl <- caret::trainControl(method = "cv", number = 2,savePredictions=T)
set.seed(1)
