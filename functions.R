...
q6 <- function(variable) {as.factor(ntile(variable, 6))}
qn <- function(variable,n) {as.factor(ntile(variable, n))}

qBr6 <- function(df, variable, rnd) {
  if (missing(rnd)) {
    as.character(quantile(round(df[[variable]],0),
                          c(.01,.17,.33,.50,.67,.83), na.rm=T))
